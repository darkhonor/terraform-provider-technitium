...
* `change_acknowledgment` - (Optional, String) Operator acknowledgment authorizing a destructive DNSSEC transition on this zone. Set to `"<ALGORITHM>/<CURVE>"` (e.g. `"ECDSA/P384"`; bare `"RSA"` for RSA) to authorize an unsign/re-sign to those parameters, or `"unsigned"` to authorize unsigning (standing consent — remove after use; a stale value draws a removal warning on every plan except under `silent` enforcement). Required for in-place `algorithm`/`curve` changes in every posture, and for unsigning under `strict` enforcement. See the [STIG Compliance Guide](../guides/stig-compliance.md).
